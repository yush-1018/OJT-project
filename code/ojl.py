...
class StorageManager:
    ...
    def update_metrics(self, game_result: Dict):
        ...
        try:
            ...
            self._apply_metrics_update(metrics, game_result)
            
            self._write_json(self.metrics_file, metrics)
        except Exception as e:
            logging.error(f"Failed to update metrics: {e}")

    def _apply_metrics_update(self, metrics: Dict, game_result: Dict):
        """Helper to apply updates to a metrics dict (for batch processing)"""
    ...
    def batch_update_metrics(self, results: List[Dict], strategy_name: str, range_config: List[int]):
        """Batch update metrics for parallel execution results"""
        try:
            metrics = self._read_json(self.metrics_file) or {}
            
            for result in results:
                # Reconstruct necessary game_data format for the helper
                game_data = {
                    'guess_count': result['guesses'],
                    'won': result['won'],
                    'strategy': strategy_name
                }
                self._apply_metrics_update(metrics, game_data)
                
            self._write_json(self.metrics_file, metrics)
        except Exception as e:
            logging.error(f"Failed to batch update metrics: {e}")
...
class ProbabilityGuidedStrategy(GuessingStrategy):
    ...
    def update(self, guess: int, result: str):
        ...
        if 0 <= idx < len(self.probabilities):
            if result == "higher":
...
class GameEngine:
    ...
    def __init__(self, config: Configuration, storage: StorageManager, persistence_enabled: bool = True):
        self.config = config
    ...
    def _save_game_result(self, won: bool):
        ...
        # CRITICAL: Only save to disk if persistence is enabled.
        # This prevents race conditions during parallel execution.
...
# Worker function must be top-level for pickling in some OS environments
def _run_batch_worker(config: Configuration, batch_size: int) -> List[Dict]:
    """Worker function for parallel execution"""
    # Create valid StorageManager but disable persistence in Engine
    storage = StorageManager(config.storage_path)
    # persistence_enabled=False prevents race conditions on the JSON file
    engine = GameEngine(config, storage, persistence_enabled=False)
    # Ensure no delay in parallel mode for speed
    engine.config.auto_play_delay = 0 
    ...
    def _parallel_simulation(self):
        ...
        with ProcessPoolExecutor(max_workers=workers) as executor:
            ...
            for i in range(workers):
                ...
                # Pass explicit function _run_batch_worker instead of lambda/local func
                futures.append(executor.submit(_run_batch_worker, self.config, batch_size))
        ...
        # Batch update metrics in main process (since workers didn't save)
        print("\n⏳ Updating aggregated metrics...")
        self.storage.batch_update_metrics(
            all_results, 
            self.config.strategy, 
            [self.config.min_range, self.config.max_range]
        )
